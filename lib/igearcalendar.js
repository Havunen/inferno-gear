...
export default class extends Component {
    ...
    buildCells(selectedDate, rangeStart) {
        const cells = [];

        for (let i = 0; i < 7; i++) {
            cells.push(<CellTemplate key={i} selectedDate={selectedDate} day={moment(rangeStart.add(1, 'days'))} onCellClick={this.selectDate} />);
        }
        ...
        for (let i=0; i<rowsLength; i++) {
            rows.push(
                <div key={i} class="igear-row">
                    ...
                    {this.buildCells(selectedDate, rangeStart)}
                </div>
            )
        }
                            ...
                            <select class="igear-calendar-select" value={selectedDate.month()} onChange={this.goToMonth}>
                                {(function() {
                                    ...
                                    for(let i = 0; i < 12; i++) {
                                        ...
                                        options.push(<option key={i} value={month.month()}>{month.format('MMM')}</option>);
                                    }
                                    return options;
                                })()}
                            </select>
